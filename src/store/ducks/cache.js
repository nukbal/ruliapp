...
import { select, put, buffers, eventChannel, END, takeEvery } from 'redux-saga/effects';
...
const actionHandler = {
  [actionType.ADD_IMAGE_CACHE]: (state, { payload }) => {
    const { sha, url, jobId, path } = payload;
    return { [sha]: { url, jobId, path, width: 0, height: 0, isReady: false }, ...state };
  },
